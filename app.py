# ========================================
# Gmail Mail Merge Tool - Optimized Stable Version
# ========================================
...
st.title("📧 Gmail Mail Merge Tool - Optimized Stable Version")
...
st.info("⚠️ Recommended max 70–80 contacts for smooth operation per batch.")
    ...
    if st.button("🚀 Send Emails / Save Drafts"):
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                if send_mode == "💾 Save as Draft":
                    ...
                    try:
                        ...
                        if e.resp.status in [429, 403]:
                            st.warning("⚠️ Gmail limit reached. Pausing 10 minutes...")
                            time.sleep(10 * 60)
                            continue
                        else:
                            raise

                # Apply label only once per message
                ...
                # Track ThreadId and optional Message-ID (every 10 emails)
            ...
            remaining = max(0, (len(df) - idx - 1) * delay)
            progress = int((idx + 1) / len(df) * 100)
            progress_bar.progress(progress)
            status_text.text(f"📤 Sending {idx+1}/{len(df)} | ✅ Sent: {sent_count} | ⚠️ Skipped: {len(skipped)} | ❌ Failed: {len(errors)} | ⏳ ETA: {int(remaining/60)} min")
